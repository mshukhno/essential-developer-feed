...
//
//  Created by Mikalai Shuhno on 02/11/2024.
//
...
final class EssentialFeedAPIEndToEndTests: XCTestCase {
    ...
    private func getFeedResult(
        ...
        let exp = expectation(description: "Wait for load competion")
        
        var receivedResult: FeedLoader.Result?
        client.get(from: feedTestServerURL) { result in
            receivedResult = result.flatMap { data, resopnse in
                do {
                    return .success(try FeedItemsMapper.map(data, from: resopnse))
                } catch {
